...
def build_ham_dataloaders(cfg: Dict[str, Any]):
    ...
    train_ds = CsvImageDataset(train_csv, img_root, classes, transform=train_tf)
    val_ds = CsvImageDataset(val_csv, img_root, classes, transform=eval_tf)
    test_ds = CsvImageDataset(test_csv, img_root, classes, transform=eval_tf) if test_csv else None
...
def build_model(cfg: Dict[str, Any], num_classes: int) -> torch.nn.Module:
    ...
    blip_cfg = model_cfg.get("blip2", {}) or {}

    # defaults
    #lavis_name = blip_cfg.get("lavis_name", "blip2_opt")
    model_type = blip_cfg.get("model_type", "pretrain_opt2.7b")
    ...
    return Blip2Classifier(
        ...
        device=device,
        train_qformer=bool(blip_cfg.get("train_qformer", False)),
        #train_vision=bool(blip_cfg.get("train_vision", False)),
        #unfreeze_vision_last_n=int(blip_cfg.get("unfreeze_vision_last_n", 0) or 0),
        #pooling=str(blip_cfg.get("pooling", "mean")),
        #head_hidden=int(blip_cfg.get("head_hidden", 0) or 0),
        #activation=str(blip_cfg.get("activation", "gelu")),
        #dropout=float(blip_cfg.get("dropout", 0.0) or 0.0),
    )
...
def main():
    ...
    ap.add_argument("--mode", required=True, choices=["train_multiclass", "eval_multiclass", "zeroshot_multiclass"])
    args = ap.parse_args()
