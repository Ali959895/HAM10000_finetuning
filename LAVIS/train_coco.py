...
# --- HARD DISABLE wandb service on HPC ---
os.environ.setdefault("WANDB_MODE", "offline")
os.environ.setdefault("WANDB_DIR", "/scratch/ali95/wandb")
...
class COCOMultiLabel(Dataset):
    def __init__(self, img_root, ann_path, vis_processor):
    ...
    def __getitem__(self, idx):
        ...
        for a in anns:
            label[self.catid_to_idx[a["category_id"]]] = 1.0
...
class VLClassifier(nn.Module):
    ...
    def extract_pooled_image_features(self, images: torch.Tensor) -> torch.Tensor:
        """
        images: [B, 3, H, W] float tensor from DataLoader
        returns: [B, D] pooled visual features (fp32)
        """
        images = images.to(self.device, non_blocking=True)

        # Match vision encoder dtype (fp16 on GPU)
        ve = self.model.visual_encoder
        ve_dtype = next(ve.parameters()).dtype
        images = images.to(dtype=ve_dtype)

        with torch.cuda.amp.autocast(enabled=images.is_cuda):
            # EVA ViT forward → [B, N, D]
            vision_embeds = ve(images)

            # Apply LN if present (BLIP-2 has ln_vision)
            if hasattr(self.model, "ln_vision") and self.model.ln_vision is not None:
                vision_embeds = self.model.ln_vision(vision_embeds)

            # Mean-pool tokens
            pooled = vision_embeds.mean(dim=1)

        return pooled.float()  # classifier runs in fp32


    def forward(self, images: torch.Tensor) -> torch.Tensor:
        feats = self.extract_pooled_image_features(images)  # [B, D]
        self._build_head_if_needed(feats.shape[-1])
...
@torch.no_grad()
def evaluate(model, loader, device, num_classes, threshold=0.5):
    model.eval()

    loss_meter = []

    metrics = {
        "f1_micro": MultilabelF1Score(num_classes, average="micro", threshold=threshold).to(device),
        "f1_macro": MultilabelF1Score(num_classes, average="macro", threshold=threshold).to(device),
        "precision_micro": MultilabelPrecision(num_classes, average="micro", threshold=threshold).to(device),
        "recall_micro": MultilabelRecall(num_classes, average="micro", threshold=threshold).to(device),
        "accuracy": MultilabelAccuracy(num_classes, average="macro", threshold=threshold).to(device),
        "map": MultilabelAveragePrecision(num_labels=num_classes, average="macro").to(device),
    }
    ...
    return {
        "val/loss": float(np.mean(loss_meter)),
        "val/mAP": metrics["map"].compute().item(),
        "val/f1_micro": metrics["f1_micro"].compute().item(),
        "val/f1_macro": metrics["f1_macro"].compute().item(),
        "val/precision_micro": metrics["precision_micro"].compute().item(),
        "val/recall_micro": metrics["recall_micro"].compute().item(),
        "val/accuracy_samples": metrics["accuracy"].compute().item(),
    }
...
def train(cfg: TrainConfig):
    ...
    # W&B init
    # --- W&B init (HPC-safe) ---
    wandb_mode = os.environ.get("WANDB_MODE", "offline")  # default offline on clusters
    ...
    # Build datasets
    # Build datasets (COCO official structure)
    ...
    for epoch in range(int(cfg.epochs)):
        print(f"\n[INFO] ===== Epoch {epoch}/{cfg.epochs - 1} started =====", flush=True)
        ...
        for it, (images, labels, _) in enumerate(train_loader):
            ...
            # ---- STEP HEADER ----
            if it == 0:
                print("[INFO] First batch fetched", flush=True)
                torch.cuda.synchronize()
                print("[INFO] CUDA sync BEFORE first forward", flush=True)
            ...
            if it == 0:
                torch.cuda.synchronize()
                print("[INFO] CUDA sync AFTER first forward", flush=True)
                print("[INFO] First forward DONE", flush=True)
        ...
        # ---- VALIDATION ----
        print("[INFO] Starting validation...", flush=True)
    ...
    print(f"[VAL] loss={val_metrics['val/loss']:.4f} | "
          f"mAP={val_metrics['val/mAP']:.4f} | "
          f"F1(micro)={val_metrics['val/f1_micro']:.4f} | "
          f"F1(macro)={val_metrics['val/f1_macro']:.4f}")

        
        
        
        #val_loss, val_map = evaluate(
        #    model, val_loader, device=device, num_classes=80
        #)
    
        #print(
        #    f"[INFO] Validation done | "
